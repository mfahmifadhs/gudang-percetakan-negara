...
class PetugasController extends Controller
{
	...
	// ========================================
	//     DAFTAR PENGIRIMAN DAN PENGELUARAN
	// ========================================
	...
	public function createCompleteItem($id)
	{
	...
	public function postCompleteItem(Request $request, $id)
	{
        ...
        if ($valImage->fails()) {
           return redirect('petugas/kelengkapan-barang/'. $id)->with('failed','Format foto tidak sesuai');
        	...
        	foreach($imgitem as $i => $image)
        	{

            	$getimg = DB::table('tbl_items_incoming')->where('id_item_incoming', $iditem[$i])->first();
			    if ($request->hasfile('item_img')) {
					...
					$file = $request->file('item_img');
				 	$filename = $image->getClientOriginalName();
				  	$image->move('dist/img/data-barang/', $filename);
			    ...
			    $updimage = OrderItemModel::where('id_item_incoming', $iditem[$i])
